...
namespace Crawfis
{
	namespace Graphics
	{
		...
		//
		class DepthLookAtTransformSolution : public TransformNodeBase
		{
		public:
			//
			// Constructor. 
			//
			DepthLookAtTransformSolution(std::string name, ISceneNode* subject)
				: TransformNodeBase(name, subject)
			{
				this->eyePosition = Crawfis::Math::Vector3(10, 1, 1);
			...
			virtual void Apply()
			{
				...
				modelView = MatrixStack::depthModelViewStack.top() * modelView;
				MatrixStack::depthModelViewStack.push(modelView);
				/*std::cout << "pushing into dmv\n";
				for (int i = 0; i < 4; i++)
					modelView.getRowVector(i).print();*/
			}
			// 
