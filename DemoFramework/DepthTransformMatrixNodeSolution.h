...
namespace Crawfis
{
	namespace Graphics
	{
		...
		//
		class DepthTransformMatrixNodeSolution : public TransformNodeBase
		{
		public:
			//
			// Constructor. 
			//
			DepthTransformMatrixNodeSolution(std::string name, ISceneNode* subject) : TransformNodeBase(name, subject)
			{
				matrix.identity();
			}
			// 
			...
			virtual void Apply()
			{
				Matrix4 transform = MatrixStack::depthModelViewStack.top() * matrix;
				MatrixStack::depthModelViewStack.push(transform);
				/*std::cout << "pushing transform into dmv\n";
				for (int i = 0; i < 4; i++)
					transform.getRowVector(i).print();*/
			}
			// 
